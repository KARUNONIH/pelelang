...
<div class="w-[90%] mx-auto mt-3 mb-5 ">
                ...
                @foreach (App\Models\itemModel::take(3)->where('complete_at', '>=', Carbon\Carbon::now('Asia/Jakarta'))->orderBy('id', 'DESC')->get() as $item)
                    <div
                        ...
                        <div class="p-5">
                            ...
                            <a href="{{ route('item.show', ['item_id' => $item->id]) }}"
                                class="absolute bottom-3 inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-gray-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-blue-800">
                                Bid Sekarang
    ...
    <div class="grid grid-cols-4 mt-6 gap-4 ml-2 mx-auto bg-[#ffffff]">
        ...
        <!--item -->
        @foreach (App\Models\itemModel::where('complete_at', '>=', Carbon\Carbon::now('Asia/Jakarta'))->paginate(8) as $item)
            <a href="{{ route('item.show', ['item_id' => $item->id]) }}"
    ...
    <div class="flex ">
        {!! App\Models\itemModel::where('complete_at', '>=', Carbon\Carbon::now('Asia/Jakarta'))->paginate(8)->links('partial.paginate') !!}
    </div>
</div>
<!-- Item 2 -->
